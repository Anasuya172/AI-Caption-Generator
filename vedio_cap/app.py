...
def transcribe_audio_english(audio_path):
    try:
        st.info("📝 Loading Whisper-large for English transcription... (please wait ⏳)")
        model = whisper.load_model("large")
...
def burn_subtitles(video_path, srt_path, output_path, ffmpeg_path):
    try:
        ...
        command = f'"{ffmpeg_exe}" -y -i "{video_full}" -vf "subtitles={srt_fixed}" "{output_full}"'
...
def main():
    ...
    st.markdown(
        ...
        🕒 *Note: Processing time depends on video length and system performance. Please wait patiently and avoid closing this window.*  
        """
    )
    ...
    if uploaded_video is not None:
        ...
        if st.button("🚀 **Start Processing**"):
            ...
            final_video = burn_subtitles("temp_video.mp4", srt_path, output_video, ffmpeg_path)
            if final_video:
